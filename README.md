...
By default, Argo infers the overall sequence divergences using read overlaps and determines an adaptive identify cutoff for identifying ARGs, which is *90 - 2.5 * 100 * median sequence divergence*. This is to ensure samples generated at different stage or by different platforms are comparable since they have vastly different qualities. However, if you want to suppress this option and use a fixed cutoff—for instance, 80% identity cutoff and 80% subject cover—you can run:
...
Yes, Argo can provide rough estimates of ARG abundances in terms of cpg for isolates. However, the computational time may be longer for oversampled pathogens (e.g., *Escherichia coli*, *Salmonella enterica*) which typically contain many copies of ARGs on their genomes.

### Does Argo work with assembled contigs?

No, Argo is inherently read-based and does not work with contigs.

### Why is Argo running slowly?

The computational time scales not only with the size of the sample but also with the number of ARG-containing reads. If your sample contains a large amount of *Escherichia coli*, which has many redundant sequences in GTDB, the computational time will be longer than usual.
